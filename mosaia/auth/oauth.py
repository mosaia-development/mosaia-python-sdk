...
class OAuth:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if not config.get('app_url'):
            config['app_url'] = DEFAULT_CONFIG.get('APP', {}).get('URL', 'https://mosaia.ai')
    ...
    def get_authorization_url_and_code_verifier(self) -> Dict[str, str]:
        ...
        if not self.config.get('scopes') or len(self.config['scopes']) == 0:
            raise Exception('scopes are required in OAuth config to generate authorization url and code verifier')
        
        if not self.config.get('redirect_uri'):
            raise Exception('redirect_uri is required in OAuth config to generate authorization url and code verifier')
    ...
    async def authenticate_with_code_and_verifier(self, code: str, code_verifier: str) -> MosaiaConfig:
        """
        ...
        """
        if not self.config.get('redirect_uri'):
            raise Exception('redirect_uri is required in OAuth config to authenticate with code and verifier')
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    return MosaiaConfig(
                        **{**self.config, 'api_key': session_data['access_token'], 'session': session_data}
                    )
        except Exception as error:
            raise error
