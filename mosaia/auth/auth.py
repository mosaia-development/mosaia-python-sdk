...
class MosaiaAuth:
    ...
    async def sign_in_with_password(self, email: str, password: str) -> MosaiaConfig:
        ...
        try:
            ...
            return MosaiaConfig(
                **{**self.config.__dict__, 'api_key': session['access_token'], 'session': session}
            )
        except Exception as error:
    ...
    async def sign_in_with_client(self, client_id: str, client_secret: str) -> MosaiaConfig:
        """
        ...
        try:
            ...
            return MosaiaConfig(
                **{**self.config.__dict__, 'api_key': data['access_token'], 'session': session}
            )
        except Exception as error:
            if hasattr(error, 'message'):
                raise Exception(str(error.message))
    ...
    async def refresh_token(self, token: Optional[str] = None) -> MosaiaConfig:
        ...
        """
        refresh_token = token or (self.config.session.refresh_token if self.config.session else None)
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    return MosaiaConfig(
                        **{**self.config.__dict__, 'api_key': session_data['access_token'], 'session': session_data}
                    )
        except Exception as error:
            raise error
